...
function StatusBadge({ status }) {
  const cfg = STATUS_CONFIG[status] || { label: status, color: '#6b7280', bg: '#f9fafb', border: '#e5e7eb', icon: '○' };
  return (
    <span style={{
...
function StatusTimeline({ currentStatus, history = [] }) {
  ...
  return (
    <div style={{ padding: '16px 0' }}>
      ...
      {history.length > 0 && (
        <div>
          <p style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            Historique
          </p>
              ...
              return (
                <div key={i} style={{ marginBottom: 12, position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {h.changed_by_name && (
                      <span style={{ fontSize: 12, color: '#6b7280' }}>par <strong>{h.changed_by_name}</strong></span>
                    )}
                  </div>
                  {h.comment && (
                    <p style={{ margin: '4px 0 0', fontSize: 12, color: '#374151', fontStyle: 'italic', background: '#f8fafc', borderRadius: 6, padding: '6px 10px' }}>
                      💬 {h.comment}
                    </p>
                  )}
                </div>
...
function RequestDetailModal({ request, onClose, onStatusChange, canChangeStatus }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 580, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1e3a5f, #2563eb)', padding: '24px 28px', borderRadius: '16px 16px 0 0', color: '#fff' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{request.type}</h2>
        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 16, marginBottom: 24 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', fontSize: 13 }}>
              <div><span style={{ color: '#9ca3af' }}>Date soumission</span><br /><strong>{fmtDate(request.created_at)}</strong></div>
              <div><span style={{ color: '#9ca3af' }}>Statut</span><br /><StatusBadge status={request.status} /></div>
              {request.validator_name && (
                <div><span style={{ color: '#9ca3af' }}>Traité par</span><br /><strong>{request.validator_name}</strong></div>
              )}
              {request.status_updated_at && (
                <div><span style={{ color: '#9ca3af' }}>Dernière MAJ</span><br /><strong>{fmtDate(request.status_updated_at)}</strong></div>
              )}
            </div>
            {request.description && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #e5e7eb' }}>
                <p style={{ margin: '0 0 4px', fontSize: 12, color: '#9ca3af' }}>Description</p>
                <p style={{ margin: 0, fontSize: 13, color: '#374151' }}>{request.description}</p>
              </div>
            )}
            {request.status_comment && (
              <div style={{ marginTop: 8 }}>
                <p style={{ margin: '0 0 4px', fontSize: 12, color: '#9ca3af' }}>Commentaire RH</p>
                <p style={{ margin: 0, fontSize: 13, color: '#374151', fontStyle: 'italic' }}>{request.status_comment}</p>
              </div>
            )}
          </div>
          ...
          {canChangeStatus && allowed.length > 0 && (
            <div style={{ marginTop: 20, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 10, padding: 16 }}>
              <p style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 600, color: '#0369a1' }}>Mettre à jour le statut</p>
              <select
                value={newStatus}
                onChange={e => setNewStatus(e.target.value)}
                style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #bae6fd', marginBottom: 10, fontSize: 13, background: '#fff' }}
              >
              ...
              <textarea
                ...
                rows={2}
                style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid #bae6fd', fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
              />
...
function NewRequestModal({ onClose, onCreated }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 480, boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        <div style={{ background: 'linear-gradient(135deg, #1e3a5f, #2563eb)', padding: '24px 28px', borderRadius: '16px 16px 0 0', color: '#fff' }}>
        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Description / Motif</label>
            <textarea
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button onClick={onClose} style={{ padding: '9px 18px', background: '#f3f4f6', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>Annuler</button>
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1000, margin: '0 auto', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#111827' }}>Demandes RH</h1>
          <p style={{ margin: '4px 0 0', fontSize: 14, color: '#6b7280' }}>Suivi de vos demandes auprès du service RH</p>
        </div>
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          {canChangeStatus && (
            <div style={{display:'flex',background:'#1e2535',borderRadius:10,padding:3,gap:2}}>
              {[{key:'list',label:'📋 Liste'},{key:'stats',label:'📊 Statistiques'}].map(t=>(
      ...
      {activeTab === 'stats' && canChangeStatus ? <StatsPanel /> : <>
      {/* Cartes stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Total',          value: stats.total,       color: '#6366f1', bg: '#f5f3ff', key: '' },
          { label: 'En cours',       value: stats.pending,     color: '#f59e0b', bg: '#fffbeb', key: 'pending' },
          { label: 'Prise en charge',value: stats.in_progress, color: '#3b82f6', bg: '#eff6ff', key: 'in_progress' },
          { label: 'Clôturées',      value: stats.closed,      color: '#10b981', bg: '#f0fdf4', key: 'closed' },
          { label: 'Rejetées',       value: stats.rejected,    color: '#ef4444', bg: '#fef2f2', key: 'rejected' }
        ].map(s => (
        ...
        {[{ value: '', label: 'Toutes' }, ...STEPS.map(s => ({ value: s.key, label: s.label })), { value: 'rejected', label: '❌ Rejetées' }].map(f => (
          <button key={f.value}
            ...
            style={{
              ...
              background: filterStatus === f.value ? '#2563eb' : '#f3f4f6',
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    <span style={{ fontWeight: 600, fontSize: 14, color: '#111827' }}>{req.type}</span>
                  ...
                  {req.description && (
                    <p style={{ margin: '4px 0 0', fontSize: 12, color: '#9ca3af', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 400 }}>
                      {req.description}
                    </p>
                  )}
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <p style={{ margin: 0, fontSize: 11, color: '#9ca3af' }}>{fmtDate(req.created_at)}</p>
