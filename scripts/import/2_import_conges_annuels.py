...
# Supprimer anciens imports
cur.execute("DELETE FROM leave_requests WHERE reason='Import SMT_V9SQL'")
print(f"Anciens imports supprimes: {cur.rowcount}")

ok = skip = 0

with open('/tmp/recap5.csv', encoding='utf-8-sig') as f:
    for line in f:
...
conn.commit()
print(f"Importes: {ok}, ignores: {skip}")

# Recalculer annual_taken
cur.execute("""
    ...
    SET annual_taken = (
        ...
        WHERE lr.employee_id = lb.employee_id
          ...
          AND lt.code IN ('0454','0455')
          AND lr.start_date >= '2025-06-01'
          AND lr.start_date <= '2026-05-31'
    )
    WHERE lb.year = 2026
""")
conn.commit()
...
cur.execute("""
    ...
    WHERE lb.year=2026 AND lb.annual_total > 0
    ORDER BY e.matricule LIMIT 10
""")
print("\nVerification:")
